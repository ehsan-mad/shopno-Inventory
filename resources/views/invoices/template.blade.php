...
<head>
    ...
    <script>
        function downloadCurrentInvoice() {
            // Extract sale ID from URL params or from a global variable
            const urlParams = new URLSearchParams(window.location.search);
            const saleId = urlParams.get('sale_id') || window.currentSaleId;
            
            if (saleId && window.parent && window.parent.downloadInvoice) {
                // If we're in a modal, call the parent's download function
                window.parent.downloadInvoice(saleId);
            } else {
        ...
        <div class="no-print mb-6 flex justify-between items-center">
            ...
            <div class="space-x-3">
                <button onclick="window.print()" class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg">
                    ...
                    Print
                </button>                <button onclick="downloadCurrentInvoice()" class="bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded-lg">
                    <svg class="inline w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
